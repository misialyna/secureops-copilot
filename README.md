...
flowchart TD
    ...
    propose -->|no active action justified| END((end))
    propose -->|action(s) proposed| gate["approval_gate (interrupt)"]
    gate -->|resume with approvals| END
```
...
sequenceDiagram
    ...
    Graph-->>API: status="completed", audit_log=[...]
    API-->>Analyst: firewall commands (if approved) + full audit log
```
...
# 5. Re-check the result at any time (e.g. after restarting the server)
